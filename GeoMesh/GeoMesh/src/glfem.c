...
double      current_text_color[4]   = {1.0,0.0,0.0,1.0};
double      current_color[4]        = {0.0,0.0,0.0,1.0};
...
void glfemChangeState(int *state, int nState){
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_J) == GLFW_PRESS) {*state = 0;}
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_K) == GLFW_PRESS) {*state = 1;}
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_L) == GLFW_PRESS) {*state = 2;}
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_M) == GLFW_PRESS) {*state = 3;}
    *state = *state<nState?*state:nState-1;
}

void glfemChangeBool(int *state){
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_I) == GLFW_PRESS) {*state = 0;}
    if (glfwGetKey(theCurrentWindow,GLFW_KEY_O) == GLFW_PRESS) {*state = 1;}
}
...
//
    ...
    if (h >= 0) {
        ...
        if (strlen(message)+off > 0) 
            glCallLists((GLsizei) strlen(message)+off, GL_UNSIGNED_BYTE, (GLubyte *) &message[-off]); }
...
void glfemReshapeWindows(femNodes *theNodes, int w, int h)
{
    ...
    double maxX  = femMax(theNodes->X,theNodes->nNodes);
    double minY  = femMin(theNodes->Y,theNodes->nNodes);
    double maxY  = femMax(theNodes->Y,theNodes->nNodes);
    double sizeX = (maxX-minX)/1.6;
    double meanX = (maxX+minX)/2.0; 
    double sizeY = (maxY-minY)/1.6;
...
{
    int j;
    if (n==1){glPointSize(10);glBegin(GL_POINTS);}
    else if (n==2){glBegin(GL_LINE_STRIP); glLineWidth(1000*current_line_width);}
    else if (n==3){glBegin(GL_LINE_STRIP); glLineWidth(1000*current_line_width);}
    else {Error("Unvalid Element\n");}
    glColor3f(current_color[0],current_color[1],current_color[2]);
    for (j = 0; j < n; j++) {      
        glVertex2f(x[j],y[j]); }
    glVertex2f(x[0],y[0]);
...
{   
    ...
    int j;
    if (n==1){glPointSize(10);glBegin(GL_POINTS);}
    else if (n==2){glBegin(GL_LINE_STRIP);}
    else if (n==3){glBegin(GL_POLYGON);}
    else {Error("Unvalid Element\n");}
    for (j=0; j < n; ++j) {
...
void glfemPlotMesh(femMesh *theMesh)
{
    ...
    double  xLoc[nLocalNode];
    double  yLoc[nLocalNode];
...
void glfemPlotDomain(femDomain *theDomain){
    ...
    for(int i=0; i < theDomain->nElem; ++i){
        ...
        for (j=0; j < nLocalNode; ++j) {
            xLoc[j] = theMesh->nodes->X[nodes[j]];
            yLoc[j] = theMesh->nodes->Y[nodes[j]];                  
            uLoc[j] = glScale(0.,1.,1.);}
        glfemDrawSolution(xLoc,yLoc,uLoc,nLocalNode); }
}
...
void glfemPlotField(femMesh *theMesh, double *u)
{
    int i,j,*nodes;
    int nLocalNode = theMesh->nLocalNode;
    double uMax = u[0], uMin = u[0];
    for (i = 0; i < theMesh->nElem; ++i){
        nodes = &(theMesh->elem[i*nLocalNode]);
        for(int j=0; j < nLocalNode; ++j){
            uMax = fmax(uMax, u[nodes[j]]);
            uMin = fmin(uMin, u[nodes[j]]);
        }
    }
    double  xLoc[nLocalNode];
    double  yLoc[nLocalNode];
